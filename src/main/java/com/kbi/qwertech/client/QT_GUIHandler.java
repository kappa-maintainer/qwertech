...
public class QT_GUIHandler {
    ...
    public static ResourceLocation backgroundIcons = new ResourceLocation(
        "qwertech:textures/gui/effect/backgrounds.png");
    ...
    public QT_GUIHandler() {
        ...
        QT_GUIHandler.colors = new short[][] { { 255, 255, 255, 255 }, { 255, 255, 255, 255 }, { 255, 255, 255, 255 },
    ...
    private void addIcon(String string, Integer[] pixels, ResourceLocation texture) {
        iconUVs.put(
            string,
            new Float[] { calculate(texture, pixels[0], false), calculate(texture, pixels[1], true),
                calculate(texture, pixels[2], false), calculate(texture, pixels[3], true) });
    }
    ...
    public void renderIcon(int xpos, int ypos, float uMin, float vMin, float uMax, float vMax, int xsize, int ysize) {
        ...
        tessellator
            .addVertexWithUV((double) (xpos + 0), (double) (ypos + ysize), RenderItem.getInstance().zLevel, uMin, vMax);
        tessellator.addVertexWithUV(
            (double) (xpos + xsize),
            (double) (ypos + ysize),
            RenderItem.getInstance().zLevel,
            uMax,
            vMax);
        tessellator
            .addVertexWithUV((double) (xpos + xsize), (double) (ypos + 0), RenderItem.getInstance().zLevel, uMax, vMin);
        tessellator
            .addVertexWithUV((double) (xpos + 0), (double) (ypos + 0), RenderItem.getInstance().zLevel, uMin, vMin);
        tessellator.draw();
    }
    ...
    public void renderIcon(int x, int y, int order, int amount, Float[] UVs, int xsize, int ysize) {
        ...
        if (isHorizontal) {
            xpos = renderRight == 0 ? xpos + (order * 20)
                : renderRight == 1 ? (xpos - ((amount - 1) * 10) + (order * 20)) : xpos - (order * 20);
        } else {
            ypos = renderDown == 0 ? ypos + (order * 20)
                : renderDown == 1 ? (ypos - ((amount - 1) * 10) + (order * 20)) : ypos - (order * 20);
        }
    ...
    public void onRendertext(RenderGameOverlayEvent.Text event) {
        ...
        if (QTConfigs.enableArmor && event.type == ElementType.TEXT) {
            ...
            Object[] potions = minecraft.thePlayer.getActivePotionEffects()
                .toArray();
            ...
            if (heartcheck
                && (minecraft.thePlayer.isPotionActive(Potion.field_76444_x) || minecraft.thePlayer.getAir() < 300)) {
                y = y - 10;
            }
            ...
            if (QTConfigs.effectBackgroundType != -1) {
                ...
                if (minecraft.thePlayer.isBlocking()) {
                    ...
                    renderIcon(
                        x,
                        y,
                        2 + potions.length + (minecraft.thePlayer.isBurning() ? 1 : 0),
                        2 + potions.length + extras,
                        bgSpots,
                        20,
                        20);
                }
                if (minecraft.thePlayer.isSneaking()) {
                    ...
                    renderIcon(
                        ...
                        y,
                        2 + potions.length
                            + (minecraft.thePlayer.isBurning() ? 1 : 0)
                            + (minecraft.thePlayer.isBlocking() ? 1 : 0),
                        2 + potions.length + extras,
                        bgSpots,
                        20,
                        20);
            ...
            String protect = protec < 1 ? "shieldNone"
                : protec < 16 ? "shieldWood"
                    : protec < 33 ? "shieldBronze" : protec < 50 ? "shieldIron" : "shieldPurple";
            ...
            String weighty = weight < 1 ? "weightNone"
                : weight < 20 ? "weightLight"
                    : weight < 40 ? "weightLittle" : weight < 70 ? "weightSignificant" : "weightMuch";
            ...
            for (int q = 0; q < potions.length; q++) {
                if (potions[q] instanceof PotionEffect) // it should always be, but we all know how things can be...
                {
                    ...
                    if (lePot.hasStatusIcon()) {
                        ...
                        int startX = l % 8 * 18;
                        int startY = l / 8 * 18;
                        Float[] borders = new Float[] { startX * 0.00390625F, (198 + startY) * 0.00390625F,
                            (18) * 0.00390625F, (18) * 0.00390625F };
            ...
            if (minecraft.gameSettings.advancedItemTooltips) {
                if (protec > 0) {
                    minecraft.fontRenderer.drawString(
                        (int) (protec) + "%",
                        x + 18 - (minecraft.fontRenderer.getStringWidth((int) (protec) + "%")),
                        y - 28,
                        16777215,
                        true);
                }
                ...
                if (weight > 0) {
                    ...
                    minecraft.fontRenderer.drawString(
                        drawit,
                        x + 44 - (minecraft.fontRenderer.getStringWidth(drawit)),
                        y - 28,
                        16777215,
                        true);
                }
