...
public class CraftingTableRenderer extends TileEntitySpecialRenderer {
    ...
    public void renderItemIn2D(Tessellator tess, float maxU, float minV, float minU, float maxV, int width, int height,
        float scale) {
    ...
    public void renderItem(ItemStack item, float rotation, double x, double y, double z, World world, float scale,
        float transparency) {
        GL11.glPushMatrix();
        ...
        if (item.getItemSpriteNumber() == 0 && leItem instanceof ItemBlock
            && RenderBlocks.renderItemIn3d(
                Block.getBlockFromItem(leItem)
                    .getRenderType())) {
            ...
            for (int q = 0; q < passes; q++) {
                ...
                if (transparency < 1) {
                    ...
                    ItemRenderer
                        .renderItemIn2D(tessellator, f1, f2, f, f3, iicon.getIconWidth(), iicon.getIconHeight(), 0.1F);
                }
                GL11.glPopMatrix();
    ...
    public void renderTileEntityAt(CraftingTableT1 tileEntity, double x, double y, double z, float partialTick) {
        ...
        float timeLoop = (tileEntity.getWorld()
            .getWorldTime() % 360) + partialTick;
        float timeSine = (tileEntity.getWorld()
            .getWorldTime() % 180) + partialTick;
        ...
        if (dist < 16) {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    ...
                    if (display != null) {
                        if (tileEntity.mFacing == CS.SIDE_Y_NEG || tileEntity.mFacing == CS.SIDE_X_NEG
                            || tileEntity.mFacing == CS.SIDE_Z_NEG) {
                            renderItem(
                                display.copy(),
                                0,
                                0.5 - (0.2 * (j - 1)),
                                1.55 - (i * 0.2) - (timeSine / 4000),
                                0.51,
                                tileEntity.getWorld(),
                                1,
                                1F);
