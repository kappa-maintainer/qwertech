...
public class FOOD {
    ...
    public final static int VERY_BAD = 1, BAD = 32, AVERAGE = 64, GOOD = 96, GREAT = 128, FABULOUS = 160,
        EXTRAORDINARY = 192, UNSURPASSED = 224, BEST = 255, PINCH = 1, DASH = 8, SPOONFUL = 16, LUMP = 32, CUPFUL = 48,
        PORTION = 64, POUND = 100, SERVING = 125, SLAB = 175, TON = 222;
    ...
    public static int getDefaultQuantity(ItemStack item) {
        if (defaultValues.containsKey(item.getUnlocalizedName())) {
            ...
            if (baseItem instanceof ItemTool || baseItem instanceof MultiItemTool
                || baseItem.isItemTool(item)
                || baseItem.isDamageable()
                || baseItem.hasContainerItem(item)) return 0;
