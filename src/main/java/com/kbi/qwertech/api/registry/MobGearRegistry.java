...
public class MobGearRegistry {
    ...
    /**
     * DO NOT CREATE A NEW REGISTRY. Call MobGearRegistry.easy, MobGearRegistry.normal, or MobGearRegistry.hard, or use
     * one of the static helper functions.
     */
    ...
    public static boolean setChances(String key, Float chance, int difficulty) {
        switch (difficulty) {
            ...
            default:
                return MobGearRegistry.easy.setChances(key, chance) && MobGearRegistry.normal.setChances(key, chance)
                    && MobGearRegistry.hard.setChances(key, chance);
        }
    }
    ...
    public static <T extends Entity> boolean registerMobKey(Class<T> mob, String key, int difficulty) {
        try {
            switch (difficulty) {
                ...
                default:
                    return MobGearRegistry.easy.registerMobKey(mob, key)
                        && MobGearRegistry.normal.registerMobKey(mob, key)
                        && MobGearRegistry.hard.registerMobKey(mob, key);
            }
        } catch (Exception e) {
            System.out.println("Unable to add MobGearRegistry entry " + key + ": " + e);
        }
    ...
    public static boolean registerTool(String key, ItemStack item, int chances, int difficulty) {
        switch (difficulty) {
            ...
            default:
                return MobGearRegistry.easy.registerTool(key, item, chances)
                    && MobGearRegistry.normal.registerTool(key, item, chances)
                    && MobGearRegistry.hard.registerTool(key, item, chances);
        }
    }
    ...
    public static boolean removeTool(String key, ItemStack item, int difficulty) {
        switch (difficulty) {
            ...
            default:
                return MobGearRegistry.easy.removeTool(key, item) && MobGearRegistry.normal.removeTool(key, item)
                    && MobGearRegistry.hard.removeTool(key, item);
        }
    }
    ...
    public static boolean isRegistered(String key, int difficulty) {
        switch (difficulty) {
            ...
            default:
                return MobGearRegistry.easy.isRegistered(key) && MobGearRegistry.normal.isRegistered(key)
                    && MobGearRegistry.hard.isRegistered(key);
        }
    }
