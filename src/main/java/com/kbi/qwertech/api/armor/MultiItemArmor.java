...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "ic2.api.item.ISpecialElectricItem", modid = CS.ModIDs.IC2),
        @Optional.Interface(iface = "ic2.api.item.IElectricItemManager", modid = CS.ModIDs.IC2),
        @Optional.Interface(iface = "micdoodle8.mods.galacticraft.api.item.IItemElectric", modid = CS.ModIDs.GC),
        @Optional.Interface(iface = "thaumcraft.api.IGoggles", modid = CS.ModIDs.TC),
        @Optional.Interface(iface = "thaumcraft.api.nodes.IRevealer", modid = CS.ModIDs.TC),
        @Optional.Interface(iface = "thaumcraft.api.IVisDiscountGear", modid = CS.ModIDs.TC) })
public class MultiItemArmor extends ItemArmor implements IItemProjectile, IItemUpdatable, IItemGT, IItemNoGTOverride,
    IItemEnergy, thaumcraft.api.IGoggles, thaumcraft.api.nodes.IRevealer, thaumcraft.api.IVisDiscountGear {
    ...
    public static final ItemStack setUpgrade(ItemStack aStack, short aUpgradeID, int aSlot) {
        ...
        if (aNBT != null) {
            ...
            if (bNBT != null) {
                ...
                if (upgrade != null && upgrade.isCompatibleWith(aStack)
                    && upgrade.baseUpgrades() + 3 + getPrimaryMaterial(aStack).mToolQuality > aSlot) {
        ...
        if (aNBT != null) {
            ...
            for (int q = 0; q < 16; q++) {
                if (cNBT.hasKey(Integer.toString(q))) {
                    IArmorUpgrade upgrade = ArmorUpgradeRegistry.instance
                        .getUpgrade(cNBT.getShort(Integer.toString(q)));
                    if (upgrade != null) {
                        returnable = returnable
                            + (upgrade.getMaxDurabilityMultiplier() * upgrade.getMaterial().mToolDurability);
                    }
                }
            }
        }
    ...
    protected static final ArmorMaterial customMat = EnumHelper
        .addArmorMaterial("QWERTECH", 32000, new int[] { 0, 0, 0, 0 }, 0);
    ...
    public MultiItemArmor(String aModID, String aUnlocalized, String aEnglish, String aEnglishTooltip) {
        ...
        try {
            ...
            energyRatio = energyHandler.getField("IC2_RATIO")
                .getFloat(null);
        } catch (Exception e) {
        ...
        if (tMaxDamage > 0 && tStats != null) {
            aList.add(
                LH.Chat.WHITE + "Durability: "
                    ...
                    + LH.Chat.GRAY);
            aList.add(
                LH.Chat.WHITE + tMaterial.getLocal()
                    + LH.Chat.YELLOW
                    + " lvl "
                    + (tMaterial.mToolQuality + tStats.getBaseQuality())
                    + LH.Chat.GRAY);
            IArmorUpgrade[] upgrades = getUpgrades(aStack);
            for (int q = 0; q < 16; q++) {
                if (upgrades[q] != null) {
    ...
    public final ItemStack addArmor(int aID, String aEnglish, String aArmorTip, IArmorStats aArmorStats,
        ...
        if (aID >= 0 && aID < 32766) {
            ...
            for (Object tRandomParameter : aRandomParameters) {
                ...
                else if (tRandomParameter instanceof ItemStackSet) ((ItemStackSet) tRandomParameter).add(rStack.copy());
    ...
    public final void addInformation(ItemStack aStack, EntityPlayer aPlayer, List aList, boolean aF3_H) {
        ...
        if (tEnergyStats != null) {
            if (tEnergyStats instanceof EnergyStatDebug) {
                ...
                for (TagData tEnergyType : tEnergyStats.getEnergyTypes(aStack)) {
                    ...
                    aList.add(
                        LH.Chat.WHITE + Math.min(tCapacity, tEnergyStats.getEnergyStored(tEnergyType, aStack))
                            + " / "
                            + tCapacity
                            + " "
                            + tEnergyType.mChatFormat
        ...
        if (tStats != null && tStats[0] > 0) {
            ...
            aList.add(
                LH.Chat.BLUE + ((tFluid == null ? "No Fluids Contained" : UT.Fluids.name(tFluid, T))) + LH.Chat.GRAY);
    ...
    public float discharge(ItemStack aStack, float aEnergy, boolean aDoExtract) {
        ...
        return useEnergy(TD.Energy.EU, aStack, tAmount, null, null, null, 0, 0, 0, F)
            && useEnergy(TD.Energy.EU, aStack, tAmount, null, null, null, 0, 0, 0, T) ? tAmount * energyRatio : 0;
    }
                ...
                if (tStats == null) {
                    ...
                    if (ST.invalid(tBroken) || tBroken.stackSize <= 0) {
                        ...
                        if (!aPlayer.worldObj.isRemote)
                            UT.Inventories.addStackToPlayerInventoryOrDrop((EntityPlayer) aPlayer, tBroken, F);
                        aStack.stackSize = 0;
                    } else {
                        if (tBroken.stackSize > 64) tBroken.stackSize = 64;
    ...
    public final ItemStack getArmorWithStats(int aArmorID, int aAmount, OreDictMaterial aPrimaryMaterial,
        ...
        if (tArmorStats != null) {
            ...
            if (aPrimaryMaterial != null) {
                if (aPrimaryMaterial.mID > 0) tArmorNBT.setShort("a", aPrimaryMaterial.mID);
                else tArmorNBT.setString("b", aPrimaryMaterial.toString());
                UT.NBT.setNumber(
                    tArmorNBT,
                    "j",
                    ((aPrimaryMaterial.mToolDurability * tArmorStats.getMaxDurabilityMultiplier())));
            }
            if (aMaxCharge > 0) {
    ...
    public int getColorFromItemStack(ItemStack aStack, int aRenderPass) {
        ...
        if (tStats != null && tStats.getRenderPasses() > aRenderPass)
    ...
    public float getElectricityStored(ItemStack aStack) {
        return getEnergyStored(TD.Energy.EU, aStack) * energyRatio;
    }
    ...
    public float getMaxElectricityStored(ItemStack aStack) {
        return getEnergyCapacity(TD.Energy.EU, aStack) * energyRatio;
    }
    ...
    public final void getSubItems(Item var1, CreativeTabs aCreativeTab, List aList) {
        OreDictMaterial[] example = new OreDictMaterial[] { MT.Al, MT.Bronze, MT.Cu, MT.Electrum, MT.Au, MT.Fe, MT.Pb,
            MT.Ni, MT.ObsidianSteel, MT.Plastic, MT.Rubber, MT.Steel, MT.Ti, MT.U_235, MT.Vibramantium,
            MT.WroughtIron };
    ...
    public boolean isItemStackUsable(ItemStack aStack) {
        ...
        for (ObjectStack<Enchantment> tEnchantment : aMaterial.mEnchantmentArmors) {
            ...
            if (tEnchantment.mObject == Enchantment.fortune)
                tMap.put(Enchantment.looting.effectId, tEnchantment.amountInt());
            if (tEnchantment.mObject == Enchantment.knockback)
                tMap.put(Enchantment.punch.effectId, tEnchantment.amountInt());
            if (tEnchantment.mObject == Enchantment.fireAspect)
                tMap.put(Enchantment.flame.effectId, tEnchantment.amountInt());
        }
        ...
        for (int i = 0; i < allEnchants.size(); i++) if (allLevels.get(i) > 0) {
            ...
            tMap.put(
                selected.effectId,
                tLevel == null ? select : tLevel == select ? tLevel + 1 : Math.max(tLevel, select));
        }
    ...
    public boolean onArmorHit(World world, EntityLivingBase entity, ItemStack stack, int armorSlot, DamageSource source,
        float amount, LivingHurtEvent event) {
    ...
    public boolean onClickedWearing(ItemStack armorStack, int slot, World world, EntityPlayer player,
        PlayerInteractEvent.Action action, int x, int y, int z, int face, PlayerInteractEvent event) {
