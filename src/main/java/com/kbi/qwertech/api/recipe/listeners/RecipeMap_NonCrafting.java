...
public class RecipeMap_NonCrafting implements IRecipeMapHandler {
    ...
    public boolean addRecipesUsing(RecipeMap aMap, boolean aNEI, ItemStack aStack, OreDictItemData aData) {
        if (isDone()) return F;
        if (isInputPrefix) {
            return aData != null && aData.hasValidMaterialData()
                && aData.mPrefix == mInput
    ...
    public boolean addAllRecipes(RecipeMap aMap) {
        ...
        if (this.isInputPrefix) {
            for (OreDictMaterial tMaterial : ((OreDictPrefix) mInput).mRegisteredMaterials)
                addRecipeForMaterial(aMap, tMaterial);
    ...
    public boolean addRecipeItemsOnly(RecipeMap aMap) {
        if (ST.invalid((ItemStack) mInput) || (ST.invalid((ItemStack) mOutput))
            || this.isInputPrefix
            || this.isOutputPrefix) {
            return F;
        }
