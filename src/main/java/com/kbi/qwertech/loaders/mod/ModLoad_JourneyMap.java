...
public class ModLoad_JourneyMap extends ModLoadBase {
    ...
    static void copyFromZip(String zipFilePath, String zipEntryName, File destDir, boolean overWrite) throws Throwable {
        ...
        try {
            while (entry != null) {
                if (entry.getName()
                    .startsWith(zipEntryName)) {
                    File toFile = new File(
                        destDir,
                        entry.getName()
                            .split(zipEntryName)[1]);
    ...
    public static boolean isInJar() {
        URL location = QwerTech.class.getProtectionDomain()
            .getCodeSource()
            .getLocation();
    ...
    public static void copyResources(File targetDirectory, String assetsPath, String setName, boolean overwrite) {
        ...
        try {
            ...
            if (isInJar()) {
                fromPath = java.net.URLDecoder.decode(resourceDir.getPath(), "utf-8")
                    .split("file:")[1].split("!/")[0];
                // copyFromZip.invoke(null, fromPath, toPath, toDir, overwrite);
                System.out
                    .println("Attempting to call " + fromPath + " : " + toPath + " : " + toDir + " : " + overwrite);
                copyFromZip(fromPath, toPath, toDir, overwrite);
            } else {
                File fromDir = new File(
                    QwerTech.class.getResource(toPath)
                        .getFile());
