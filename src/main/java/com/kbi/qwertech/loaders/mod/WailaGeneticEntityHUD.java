...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "mcp.mobius.waila.api.IWailaEntityProvider", modid = CS.ModIDs.WAILA) })
public class WailaGeneticEntityHUD implements IWailaEntityProvider {
    ...
    public List<String> getWailaHead(Entity entity, List<String> currenttip,
        mcp.mobius.waila.api.IWailaEntityAccessor accessor, mcp.mobius.waila.api.IWailaConfigHandler config) {
        return currenttip;
    }
    ...
    public List<String> getWailaBody(Entity entity, List<String> currenttip,
        ...
        if (entity instanceof IGeneticMob) {
            ...
            String[] s = new String[] { "" + igm.getSmart(), "" + igm.getSize(), "" + igm.getStamina(),
                "" + igm.getSnarl(), "" + igm.getMaturity(), "" + igm.getFertility(), "" + igm.getStrength(),
                "" + igm.getMutable() };
            for (int q = 0; q < s.length; q++) {
                s[q] = (s[q].length() == 1 ? "    "
                    : (s[q].length() == 2 ? "   " : (s[q].length() == 3 ? "  " : (s[q].length() == 4 ? " " : ""))))
                    + s[q];
            }
