...
public class Behavior_Scrape extends AbstractBehaviorDefault {
    ...
    public boolean onLeftClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        ...
        if (mat != MT.NULL && mat != MT.Empty && mat != MT.Butter) { // if we've added any spikes, behavior is a little
                                                                     // more... destructive
            if (aEntity instanceof EntityThrowable) {
                if (aEntity instanceof EntityPotion) {// smacking a potion just applies it to the smacker
                    for (int j = 0; j < 8; ++j) {
                        aEntity.worldObj
                            .spawnParticle("bubble", aEntity.posX, aEntity.posY, aEntity.posZ, 0.0D, 0.0D, 0.0D);
                    }
                            ...
                            while (iterator1.hasNext()) {
                                ...
                                if (Potion.potionTypes[i].isInstant()) {
                                    Potion.potionTypes[i].affectEntity(
                                        ((EntityPotion) aEntity).getThrower(),
                                        aPlayer,
                                        potioneffect.getAmplifier(),
                                        1);
                                } else {
                                    aPlayer.addPotionEffect(
                                        new PotionEffect(i, potioneffect.getDuration(), potioneffect.getAmplifier()));
                                }
                            }
                        }

                        aEntity.worldObj.playAuxSFX(
                            2002,
                            (int) Math.round(aEntity.posX),
                            (int) Math.round(aEntity.posY),
                            (int) Math.round(aEntity.posZ),
                            ((EntityPotion) aEntity).getPotionDamage());
                        aEntity.setDead();
                    }
                } else {// this is egg, snowball, etc. just destroy
                    for (int j = 0; j < 8; ++j) {
                        aEntity.worldObj
                            .spawnParticle("snowballpoof", aEntity.posX, aEntity.posY, aEntity.posZ, 0.0D, 0.0D, 0.0D);
                    }
                    if (!aEntity.worldObj.isRemote) {
                        aEntity.setDead();
                    }
                }
            } else if (aEntity instanceof EntityFireball && !(aEntity instanceof EntityWitherSkull)) {// smack it to
                                                                                                      // dustereens
                for (int j = 0; j < 8; ++j) {
            ...
            } else if (aEntity instanceof IProjectile) {// anything else: arrows, bullets, rocks, whatever
                                                        // meh
            ...
            if (MobScrapeRegistry.isRegistered(aEntity.getClass())) {
                ...
                if (randomSlot != null) {// smack away a single item (not a single stack!)
                    ((EntityPlayer) aEntity).dropPlayerItemWithRandomChoice(
                        ((EntityPlayer) aEntity).inventory.decrStackSize(randomChoice, 1),
                        false);
                }
            } else if (aEntity instanceof EntityAnimal && QTConfigs.doMobScrapesDrop) {
                aEntity.entityDropItem(OP.scrapGt.mat(MT.MeatRaw, 1), 1);
            } else if (aEntity instanceof EntityMob && ((EntityMob) aEntity).isEntityUndead()
                && QTConfigs.doMobScrapesDrop) {
