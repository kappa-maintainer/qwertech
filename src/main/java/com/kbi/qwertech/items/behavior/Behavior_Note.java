...
public class Behavior_Note extends gregapi.item.multiitem.behaviors.IBehavior.AbstractBehaviorDefault
    ...
    @Override
    public ItemStack onItemRightClick(MultiItem aItem, ItemStack stack, World world, EntityPlayer player) {
        ...
        UT.Sounds
            .send(world, soundName + octave, 0.5F, pitchy, (int) player.posX, (int) player.posY, (int) player.posZ);
        progress = progress + 1;
        if (progress >= song.length) progress = 0;

        if (soundName.equals("qwertech:note.kazoo")) {
            List<Entity> entities = world
                .getEntitiesWithinAABB(EntityLivingBase.class, player.boundingBox.expand(8, 4, 8));
