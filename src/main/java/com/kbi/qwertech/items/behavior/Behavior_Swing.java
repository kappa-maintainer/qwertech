...
public class Behavior_Swing extends Behavior_AOE {
    ...
    public boolean onLeftClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        ...
        if (aEntity instanceof IProjectile) {
            String className = aEntity.getClass()
                .getName()
                .toLowerCase();
            if (className.contains("arrow") || className.contains("bullet")
                || className.contains("slug")
                || className.contains("shell")
                || className.contains("shot")
                || className.contains("pellet")
                || className.contains("ammo")
                || className.contains("skull")
                || className.contains("burst")
                || className.contains("beam")
                || className.contains("blast")
                || className.contains("missile")
                || className.contains("bomb")
                || className.contains("dynamite")
                || className.contains("nuke")) {} else { // only smack things that aren't high-powered weaponry
                Vec3 lookVector = aPlayer.getLookVec();
                ((IProjectile) aEntity)
                    .setThrowableHeading(lookVector.xCoord, lookVector.yCoord, lookVector.zCoord, 1, 1);
                aEntity.addVelocity(lookVector.xCoord, lookVector.yCoord, lookVector.zCoord);
                for (int i = 1; i < 9; i++) {
                    aEntity.worldObj.spawnParticle(
                        "townaura",
                        (float) aEntity.posX + rand.nextFloat(),
                        (float) aEntity.posY + rand.nextFloat(),
                        (float) aEntity.posZ + rand.nextFloat(),
                        aEntity.motionX / i,
                        aEntity.motionY / i,
                        aEntity.motionZ / i);
                }
                if (aEntity instanceof EntityArrow) {
                    ...
                    try {
                        ...
                        System.out.println(
                            "Attempted to set thrower of a " + aEntity.getClass()
                                .getName() + " and failed");
                        e.printStackTrace();
                    }
                }
