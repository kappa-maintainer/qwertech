...
public class QT_Tool_Mace extends ToolStats {
    ...
    public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        ...
        if (aBlock == Blocks.stone || aBlock == Blocks.cobblestone) {
            ...
            for (int q = 0; q < aDrops.size(); q++) {
                if (ANY.Stone.contains(aDrops.get(q))
                    && !(OP.dust.contains(aDrops.get(q)) || OP.dustImpure.contains(aDrops.get(q))
                        || OP.dustSmall.contains(aDrops.get(q))
                        || OP.dustTiny.contains(aDrops.get(q)))) {
    ...
    public IIconContainer getIcon(boolean aIsToolHead, ItemStack aStack) {
        return aIsToolHead
            ? MultiItemTool.getPrimaryMaterial(aStack, MT.Steel).mTextureSetsItems
                .get(gregapi.oredict.OreDictPrefix.get("toolHeadMace").mIconIndexItem)
            : MultiItemTool.getSecondaryMaterial(aStack, MT.Wood).mTextureSetsItems.get(OP.stick.mIconIndexItem);
    }

    @Override
    public short[] getRGBa(boolean aIsToolHead, ItemStack aStack) {
        return aIsToolHead ? MultiItemTool.getPrimaryMaterial(aStack, MT.Steel).mRGBaSolid
            : MultiItemTool.getSecondaryMaterial(aStack, MT.Wood).mRGBaSolid;
    }
