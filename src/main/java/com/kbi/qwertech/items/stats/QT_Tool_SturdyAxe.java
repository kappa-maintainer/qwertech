...
public class QT_Tool_SturdyAxe extends GT_Tool_Axe {
    ...
    public long checkWood(Block block, int x, int y, int z, long increment, long durability, World world,
        boolean breakIt) {
        ...
        if (aBlock != block) {
            if (!aBlock.isWood(world, x, y, z)
                && !OP.log.contains(ST.make(aBlock, 1, world.getBlockMetadata(x, y, z)))) {
        ...
        if (breakIt) {
            ...
            returnable = checkWood(block, x + 1, y, z, increment, durability, world, breakIt)
                + checkWood(block, x - 1, y, z, increment, durability, world, breakIt)
                + checkWood(block, x + 1, y, z + 1, increment, durability, world, breakIt)
                + checkWood(block, x + 1, y, z - 1, increment, durability, world, breakIt)
                + checkWood(block, x - 1, y, z + 1, increment, durability, world, breakIt)
                + checkWood(block, x - 1, y, z - 1, increment, durability, world, breakIt)
                + checkWood(block, x, y, z + 1, increment, durability, world, breakIt)
                + checkWood(block, x, y, z - 1, increment, durability, world, breakIt);
        }
        if (y < world.getHeight()) {
            y = y + 1;
            returnable = returnable + checkWood(block, x, y, z, increment, durability, world, breakIt)
                + checkWood(block, x + 1, y, z, increment, durability, world, breakIt)
                + checkWood(block, x - 1, y, z, increment, durability, world, breakIt)
                + checkWood(block, x + 1, y, z + 1, increment, durability, world, breakIt)
                + checkWood(block, x + 1, y, z - 1, increment, durability, world, breakIt)
        ...
        if (breakIt && returnable <= 0) {
            WD.leafdecay(world, x, y, z, block);
        } // trigger leaf decay at the end of a branch
    ...
    public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        ...
        if ((!aPlayer.worldObj.isRemote) && (!aPlayer.isSneaking())) {
            if (aBlock.isWood(aPlayer.worldObj, aX, aY + 1, aZ)
                || OP.log.contains(aDrops.toArray(new ItemStack[aDrops.size()]))) {
                try {
                    long tIncrement = 1L;
                    rAmount = (int) checkWood(
                        aBlock,
                        aX,
                        aY + 1,
                        aZ,
