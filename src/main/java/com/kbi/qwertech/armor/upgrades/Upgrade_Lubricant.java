...
public class Upgrade_Lubricant extends UpgradeBase {
    ...
    public void onArmorTick(World world, EntityLivingBase player, ItemStack stack) {
        int tX = MathHelper.floor_double(player.posX), tY = MathHelper.floor_double(player.boundingBox.minY - 0.001F),
            tZ = MathHelper.floor_double(player.posZ);
        ...
        if (mat == Material.iron || mat == Material.rock
            ...
            || mat == Material.wood
            || mat == Material.glass) {
            double maxSpeed = player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .getAttributeValue();
