...
public class CommonProxy extends Abstract_Proxy {
    ...
    public void onProxyAfterPostInit(Abstract_Mod aMod, FMLPostInitializationEvent aEvent) {
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(this);
        // this allows it to read game events for other classes without having to be added unnecessarily to all the game
        // building events.
    }
    ...
    public void onPlayerItemUseFinish(PlayerUseItemEvent.Finish event) {
        if (event.item != null && event.item.getItem() == CS.ToolsGT.sMetaTool
            && event.item.getItemDamage() == CS.ToolsGT.WRENCH) {
                        ...
                        if (previous.getItem() instanceof MultiItemTool) {
                            if (MultiItemTool.getToolDamage(previous) == 0) {
                                ...
                                if (previous.getItem() == CS.ToolsGT.sMetaTool) {
                                    ...
                                    switch (previous.getItemDamage()) {
                                        case 0:
                                            stack = OreDictPrefix.get("toolHeadMattock")
                                                .mat(mat, 1);
                                            break;
                                        case 6:
                                            stack = OreDictPrefix.get("toolHeadMace")
                                                .mat(mat, 1);
                                            break;
                                        default:
                                            return;
