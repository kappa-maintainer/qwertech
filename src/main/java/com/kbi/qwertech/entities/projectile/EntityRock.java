...
public class EntityRock extends EntityThrowable {
    ...
    public EntityRock(World p_i1755_1_, EntityLivingBase p_i1755_2_, float p_i1755_4_, OreDictMaterial mt,
        short rockType) {
        ...
        this.setLocationAndAngles(
            p_i1755_2_.posX,
            p_i1755_2_.posY + p_i1755_2_.getEyeHeight(),
            p_i1755_2_.posZ,
            p_i1755_2_.rotationYaw,
            p_i1755_2_.rotationPitch);
    ...
    public void bounce(double x, double y, double z, double strength) {
        ...
        if (this.getType() == 0) {
            ...
            UT.Sounds.send(
                this.worldObj,
                Block.soundTypeMetal.getStepResourcePath(),
                0.5F,
                0.8F + new Random().nextFloat(),
                (int) this.posY,
                (int) this.posY,
    ...
    protected void onImpact(MovingObjectPosition mop) {
        ...
        if (mop.typeOfHit == MovingObjectType.ENTITY
            && (mop.entityHit != this.getThrower() || this.ticksExisted > 20)) {
            if (!(this.worldObj.isRemote)) {

                mop.entityHit
                    .attackEntityFrom(DamageSource.causeThrownDamage(this, this.getThrower()), this.getDamage());

                if (mop.entityHit instanceof EntityLivingBase) {
                    UT.Enchantments.applyBullshitA(
                        (EntityLivingBase) mop.entityHit,
                        this.getThrower() == null ? this : this.getThrower(),
                        OP.arrowGtWood.mat(this.getMaterial(), 1));
                    UT.Enchantments.applyBullshitB(
                        (this.getThrower() instanceof EntityLivingBase) ? this.getThrower() : null,
                        mop.entityHit,
                        OP.arrowGtWood.mat(this.getMaterial(), 1));
                }

                if (this.getMaterial()
                    .contains(TD.Properties.FLAMMABLE) || this.isBurning()) {
                    this.setFire(1);
                    mop.entityHit.setFire(2);
                }

                if (this.getMaterial()
                    .contains(TD.Properties.EXPLOSIVE) && QTConfigs.slingshotExplode
                    && (new Random().nextFloat() > 0.5 || this.isBurning())) {
            ...
            if (this.worldObj.getBlock(mop.blockX, mop.blockY, mop.blockZ)
                .getMaterial() == Material.glass && QTConfigs.slingshotGlass
                && (!(this.getThrower() instanceof EntityPlayerMP) || this.worldObj
                    .canMineBlock((EntityPlayer) this.getThrower(), mop.blockX, mop.blockY, mop.blockZ))) {
                ...
                if (!(this.worldObj.isRemote)
                    && ((block.getBlockBoundsMaxX() == 1 && block.getBlockBoundsMaxZ() == 1) && speed > 2)
                    || speed > 1) {
                    UT.Sounds.send(
                        this.worldObj,
                        Block.soundTypeGlass.getBreakSound(),
                        1,
                        1,
                        mop.blockX,
                        mop.blockY,
            ...
            if (this.worldObj
                .isSideSolid(mop.blockX, mop.blockY, mop.blockZ, ForgeDirection.getOrientation(mop.sideHit))) {
                bounce = true; // bounce off side
            } /*
               * else if ((Math.abs(this.motionY) < 2) && (Math.abs(this.motionX) + Math.abs(this.motionZ) > 0.5) &&
               * this.worldObj.isAnyLiquid(AxisAlignedBB.getBoundingBox(mop.blockX, mop.blockY, mop.blockZ, mop.blockX,
        ...
        if (bounce) {
            ...
            if (!this.worldObj.isRemote
                && ((Math.abs(this.motionX) + Math.abs(this.motionY) + Math.abs(this.motionZ)) < 0.1)) {
