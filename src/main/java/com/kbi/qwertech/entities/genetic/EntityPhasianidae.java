...
public class EntityPhasianidae extends EntityChicken implements IGeneticMob, GMIs.IHitAggro, GMIs.IAutoAggro {
    ...
    public boolean interact(EntityPlayer playa) {
        ...
        if (stacky.getItem() instanceof MultiItem) {
            ArrayList<IBehavior<MultiItem>> meep = ((MultiItem) stacky.getItem()).mItemBehaviors
                .get((short) stacky.getItemDamage());
            ...
            for (IBehavior beep : meep) {
                if (beep instanceof Behavior_Tool) {
                    if (((Behavior_Tool) beep).mToolName.equals("magnifyingglass")) {
                        ...
                        chats.add(
                            this.theSubtype.getTag(NAME_ENGLISH) + "("
                                + this.theSpecies.getTag(RegisterSpecies.NAME_LATIN)
                                + ")");
                        ...
                        switch (randomStat) {
                            ...
                            case 2:
                                if (this.getMaturity() < 1000) {
                                    ...
                                    chats.add(
                                        "It grows up very fast. Just yesterday it still seemed like a little egg!");
                                } else {
                                    chats.add(
                                        "What maturity! It seems to grow up faster than an orphan on the streets...");
                                }
                                break;
                            case 3:
    ...
    public EntityPhasianidae(World p_i1682_1_) {
        ...
        if ((species == -1 || subtype == -1) && !p_i1682_1_.isRemote) {
            species = 0;
            subtype = 0;
            BiomeGenBase theBiome = p_i1682_1_
                .getBiomeGenForCoords((int) Math.floor(this.posX), (int) Math.floor(this.posZ));
            ...
            if (possible.size() > 0) {
                ...
                List<Subtype> subtypes = possible.get(chosen)
                    .getSubtypesForBiome(theBiome);
                if (subtypes.size() > 0) {
                    species = chosen;
    ...
    public void assignRandomStats(Random rand, Species theSpecies, Subtype theSubtype) {
        ...
        data[0] = (short) (theSubtype.getPrefSize() + (Math
            .round((data[0] - theSubtype.getPrefSize()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[1] = (short) (rand.nextInt(theSubtype.getMaxStrength() - theSubtype.getMinStrength())
            + theSubtype.getMinStrength());
        data[1] = (short) (theSubtype.getPrefStrength() + (Math
            .round((data[1] - theSubtype.getPrefStrength()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[2] = (short) (rand.nextInt(theSubtype.getMaxStamina() - theSubtype.getMinStamina())
            + theSubtype.getMinStamina());
        data[2] = (short) (theSubtype.getPrefStamina() + (Math
            .round((data[2] - theSubtype.getPrefStamina()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[3] = (short) (rand.nextInt(theSubtype.getMaxSmart() - theSubtype.getMinSmart())
            + theSubtype.getMinSmart());
        data[3] = (short) (theSubtype.getPrefSmart() + (Math
            .round((data[3] - theSubtype.getPrefSmart()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[4] = (short) (rand.nextInt(theSubtype.getMaxSnarl() - theSubtype.getMinSnarl())
            + theSubtype.getMinSnarl());
        data[4] = (short) (theSubtype.getPrefSnarl() + (Math
            .round((data[4] - theSubtype.getPrefSnarl()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[5] = (short) (rand.nextInt(theSubtype.getMaxMutable() - theSubtype.getMinMutable())
            + theSubtype.getMinMutable());
        data[5] = (short) (theSubtype.getPrefMutable() + (Math
            .round((data[5] - theSubtype.getPrefMutable()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[6] = (short) (rand.nextInt(theSubtype.getMaxFertility() - theSubtype.getMinFertility())
            + theSubtype.getMinFertility());
        data[6] = (short) (theSubtype.getPrefFertility() + (Math.round(
            (data[6] - theSubtype.getPrefFertility()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        data[7] = (short) (rand.nextInt(theSubtype.getMaxMaturity() - theSubtype.getMinMaturity())
            + theSubtype.getMinMaturity());
        data[7] = (short) (theSubtype.getPrefMaturity() + (Math
            .round((data[7] - theSubtype.getPrefMaturity()) * rand.nextFloat() * rand.nextFloat() * rand.nextFloat())));
        primaryColor = COLOR.getRandom(
            theSubtype.getTag(Integer.class, RegisterSpecies.COLOR_PRIMARY_MIN),
            theSubtype.getTag(Integer.class, RegisterSpecies.COLOR_PRIMARY_MAX));
        secondaryColor = COLOR.getRandom(
            theSubtype.getTag(Integer.class, RegisterSpecies.COLOR_SECONDARY_MIN),
            theSubtype.getTag(Integer.class, RegisterSpecies.COLOR_SECONDARY_MAX));
    ...
    protected Item getDropItem() {
        if (theSpecies.hasTag(RegisterSpecies.DROP_PRIMARY)) {
            return ((ArrayList<ItemStack>) theSpecies.getTag(RegisterSpecies.DROP_PRIMARY)).get(0)
                .getItem();
        }
        return null;
    }
    ...
    protected void dropFewItems(boolean isPlayer, int looting) {
        ...
        if (rares != null && !rares.isEmpty()) {
            ItemStack rare = rares.get(rand.nextInt(rares.size()))
                .copy();
        ...
        if (secondaries != null && !secondaries.isEmpty()) {
            ItemStack drop = secondaries.get(rand.nextInt(secondaries.size()))
                .copy();
        ...
        if (primaries != null && !primaries.isEmpty()) {
            ItemStack meat = primaries.get(rand.nextInt(primaries.size()))
                .copy();
            meat.stackSize = 1;
            ST.drop(this, meat);
        }
    ...
    public EntityPhasianidae createChild(EntityAgeable other) {
         ...
         * if (EntityHelperFunctions.doesMobFitSpecies((EntityPhasianidae)p_90011_1_,
         * MobSpeciesRegistry.getSpecies(this.getClass(), this.species), 1F) ||
         * EntityHelperFunctions.doesMobFitSpecies(this,
         * MobSpeciesRegistry.getSpecies(((IGeneticMob)p_90011_1_).getClass(),
         * ((EntityPhasianidae)p_90011_1_).getSpeciesID()), 1F))
         * {
    ...
    public boolean isBreedingItem(ItemStack stack) {
        ...
        return !theSpecies.hasTag(RegisterSpecies.BREEDING_FOOD) && !theSubtype.hasTag(RegisterSpecies.BREEDING_FOOD)
            && (OM.is("listAllseed", stack) || OP.seed.contains(stack));
    }
    ...
    public void updateHealthAndSpeed() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue((float) Math.ceil(health));
        this.setHealth((float) Math.ceil(health * currentHealth));
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(Math.max(0.05D, Math.min(speed, 0.5D)));
        this.setAIMoveSpeed((float) Math.max(0.05D, Math.min(speed, 0.5D)));
        // System.out.println("With a size of " + getSize() + ", strength of " + getStrength() + ", and stamina of " +
        // getStamina() + ", Health is now " + health + " and we adjusted speed from " + speed + " to " +
