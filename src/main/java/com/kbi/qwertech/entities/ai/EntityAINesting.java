...
public class EntityAINesting extends IMobAITimer.EntityAITimed {
            ...
            for (int x2 = -x1; x2 <= x1; x2++) {
                for (int y2 = -y1; y2 <= y1; y2++) {
                    for (int z2 = -z1; z2 <= z1; z2++) {
                        ...
                        if (!el.worldObj.getBlock(x, y + 1, z)
                            .canBeReplacedByLeaves(el.worldObj, x, y + 1, z)
                            && !el.worldObj.getBlock(x, y + 2, z)
                                .canBeReplacedByLeaves(el.worldObj, x, y + 2, z)) {
                            continue;
                        }
                        if (existing) {
                            ...
                            if (block instanceof BlockDirt || block instanceof BlockGrass
                                ...
                                for (int q = 0; q < 4; q++) {
                                    ...
                                    if (check instanceof BlockBush || check instanceof BlockCrops
                                        || check instanceof BlockReed) {
                                        if (el.getNavigator()
                                            .getPathToXYZ(x, y, z) != null) {
                                            return Vec3.createVectorHelper(x, y, z);
                                        }
    ...
    public void successfulNesting() {
        if (!laidTonight && !livingEntity.isDead
            && ourNest != null
            && livingEntity instanceof IGeneticMob
            ...
            if (igm.isFertilized()) {
                ...
                try {
                    Constructor constructor = livingEntity.getClass()
                ...
                nbt.setLong(
                    "Timer",
                    livingEntity.worldObj.getTotalWorldTime() + (Short.MAX_VALUE - (igm).getMaturity()));
    ...
    public boolean shouldExecute() {
        if (cooldown > 0) {
            ...
            if (livingEntity.worldObj.getTileEntity(
                (int) Math.floor(livingEntity.posX),
                (int) Math.floor(livingEntity.posY),
                (int) Math.floor(livingEntity.posZ)) instanceof NestTileEntity) {
                // System.out.println("We're in a nest already, hmph");
                return false;
            } else if (livingEntity.worldObj.getTileEntity(
                (int) Math.floor(livingEntity.posX),
                (int) Math.floor(livingEntity.posY - 1),
                (int) Math.floor(livingEntity.posZ)) instanceof NestTileEntity) {
                    return false;
                }
            // System.out.println("We're lookin' for a nest here at " + livingEntity.posX + ", " + livingEntity.posZ);
            Vec3 vec3;
            if (ourNest != null && ourNest.getWorld() == livingEntity.worldObj
                && livingEntity.worldObj.getTileEntity(ourNest.xCoord, ourNest.yCoord, ourNest.zCoord) == ourNest) {
    ...
    public boolean continueExecuting() {
        ...
        if (Math.floor(this.livingEntity.posX) == this.xPosition && Math.floor(this.livingEntity.posZ) == this.zPosition
            && this.livingEntity.posY - this.yPosition > -0.1
            && this.livingEntity.posY - this.yPosition < 1.1) {
            ...
            if (!(te instanceof NestTileEntity)) {
                ...
                Block block = world
                    .getBlock((int) Math.floor(xPosition), (int) Math.floor(yPosition), (int) Math.floor(zPosition));
                // we check it again to make sure that it hasn't somehow reverted while the mob was trying to pathfind
                // there
                if ((world.getBlock(x, y + 1, z)
                    .canBeReplacedByLeaves(world, x, y + 1, z)
                    && world.getBlock(x, y + 2, z)
                        .canBeReplacedByLeaves(world, x, y + 2, z))
                    && (block instanceof BlockDirt || block instanceof BlockGrass
                        || block instanceof BlockFarmland
                        || block instanceof BlockMycelium
                        || block instanceof BlockSoil
                        || block.canSustainPlant(world, x, y, z, ForgeDirection.UP, (BlockSapling) Blocks.sapling))) {
        ...
        // System.out.println("We're trying to get to " + this.xPosition + ", " + this.yPosition + ", " + this.zPosition
        // + " but we're at " + Math.floor(livingEntity.posX) + ", " + Math.floor(livingEntity.posY) + ", " +
        // Math.floor(livingEntity.posZ));
        if (this.livingEntity.getNavigator()
            .noPath()) {
            this.livingEntity.getNavigator()
                .tryMoveToXYZ(this.xPosition, this.yPosition, this.zPosition, 1);
        }
        return true;
    }
