...
public class EntityAIEatFoodOffTheGround extends EntityAIBase {
    ...
    /**
     * Initiates basic food-eating protocol. Within 16 blocks, if health is below max, half a heart per non-food
     * breeding item
     * 
    ...
    /**
     * Creates food-eating protocol that allows the animal to ignore ground food until low on health, and set how far
     * they "smell"
     * 
     * @param entity      The animal to be hungry
     * @param minHealth   How much health must be lost before they are hungry
    ...
    public boolean shouldExecute() {
        ...
        List<EntityItem> entities = ourEntity.worldObj
            .getEntitiesWithinAABB(EntityItem.class, ourEntity.boundingBox.expand(maxDist, maxDist * 0.25, maxDist));
    ...
    public boolean continueExecuting() {
        ...
        if (target == null || target.isDead
            || target.worldObj != ourEntity.worldObj
            || target.getEntityItem() == null
            || target.getEntityItem().stackSize < 1) return false;
        ...
        if (Math.floor(ourEntity.posX) == this.xPos && Math.floor(ourEntity.posZ) == this.zPos
            && ourEntity.posY - this.yPos > -0.1
            && ourEntity.posY - this.yPos < 0.5) {
            float amount = heal;
