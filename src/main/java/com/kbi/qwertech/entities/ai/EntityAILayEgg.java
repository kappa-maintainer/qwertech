...
public class EntityAILayEgg extends EntityAIBase {
    ...
    public EntityAILayEgg(EntityLivingBase entity, int minTime, int maxTime) {
        ...
        minEggTimer = minTime;
        maxEggTimer = maxTime;
        eggTimer = (int) Math.floor(
            (maxEggTimer - ourEntity.getRNG()
                .nextInt(((IGeneticMob) ourEntity).getFertility() + 1)) * 0.25)
            + minEggTimer;
        setEggData();
        setMutexBits(5);
    }
    ...
    public void startExecuting() {
        ...
        if (((IGeneticMob) ourEntity).isFertilized()) {
            ...
            try {
                Constructor constructor = ourEntity.getClass()
                ...
                EntityHelperFunctions
                    .createOffspring((IGeneticMob) returnable, (IGeneticMob) ourEntity, (IGeneticMob) fakeParent);
            ...
            nbt.setLong(
                "Timer",
                ourEntity.worldObj.getTotalWorldTime() + (Short.MAX_VALUE - ((IGeneticMob) ourEntity).getMaturity()));
            egg.setTagCompound(nbt);
        }
        // System.out.println("Should be dropping " + egg.getUnlocalizedName() + " at " + ourEntity.posX + "x, " +
        // ourEntity.posY + "y, " + ourEntity.posZ + "z");
