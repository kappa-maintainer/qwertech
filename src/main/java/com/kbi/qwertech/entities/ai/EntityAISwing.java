...
public class EntityAISwing extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (!this.thisEntity.isSwingInProgress) {
            ...
            if (is != null && is.getItem() == QTI.qwerTool.getItem()) {
                ArrayList<IBehavior<MultiItem>> behaviors = ((MultiItemTool) QTI.qwerTool.getItem()).mItemBehaviors.get(
                    (short) QTI.qwerTool.getItem()
                        .getDamage(is));
                if (behaviors != null) {
    ...
    public void startExecuting() {
        ...
        float f1 = 1F;
        List entities = thisEntity.worldObj.getEntitiesWithinAABBExcludingEntity(
            thisEntity,
            thisEntity.boundingBox.addCoord(vec31.xCoord * 2, vec31.yCoord * 2, vec31.zCoord * 2)
                .expand(f1, f1, f1)
        ...
        for (int q = 0; q < entities.size(); q++) {
            ...
            if (check instanceof IProjectile) {
                shouldSwing = true;
                thisEntity.getLookHelper()
                    .setLookPositionWithEntity(check, 10, thisEntity.getVerticalFaceSpeed());
            }
        }
        if (shouldSwing) {
