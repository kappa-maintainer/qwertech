...
public class EntityAIHydrationLimit extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (theEntity.worldObj.rand.nextInt(100) == 0) {
            ...
            if (biome.rainfall < this.minTemp && this.mode < 1
                && !this.theEntity.isInWater()
                && !(this.theEntity.worldObj.isRaining() && biome.rainfall > 0)) {
                this.theEntity.moveForward = 0;
                this.theEntity.moveStrafing = 0;
                this.theEntity.attackEntityFrom(
                    new DamageSource("dry").setDamageBypassesArmor()
                        .setFireDamage(),
                    this.damage);
            } else if ((biome.rainfall > this.maxTemp || (this.maxTemp >= 1
                && (this.theEntity.isInWater() || (this.theEntity.worldObj.isRaining() && biome.rainfall > 0))))
                && this.mode > -1) {
                    this.theEntity.moveForward = 0;
                    this.theEntity.moveStrafing = 0;
