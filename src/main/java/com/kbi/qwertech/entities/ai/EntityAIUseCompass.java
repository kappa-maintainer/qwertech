...
public class EntityAIUseCompass extends EntityAITarget {
    ...
    public boolean shouldExecute() {
        if (this.taskOwner.getHeldItem() != null && Item.getIdFromItem(
            this.taskOwner.getHeldItem()
                .getItem())
            == Item.getIdFromItem(Items.compass)) {
    ...
    public boolean continueExecuting() {
        ...
        if (counter >= 20) {
            counter = 0;
            this.taskOwner.setAttackTarget(this.taskOwner.worldObj.getClosestPlayerToEntity(this.taskOwner, 128));
            List list = this.taskOwner.worldObj
                .selectEntitiesWithinAABB(EntityLiving.class, this.taskOwner.boundingBox.expand(16, 4.0D, 16), null);
            for (int q = 0; q < list.size(); q++) {
                EntityLiving follower = (EntityLiving) list.get(q);
                if (follower instanceof EntityZombie || follower instanceof EntitySkeleton
                    || follower instanceof EntityWitch) {
                    follower.setAttackTarget(this.taskOwner.getAttackTarget());
                    follower.getEntityAttribute(SharedMonsterAttributes.followRange)
                        .setBaseValue(100.0D);
                }
            }
            for (int q = 0; q < followers.size(); q++) {
                ...
                oldFollower.getEntityAttribute(SharedMonsterAttributes.followRange)
                    .setBaseValue(40.0D);
