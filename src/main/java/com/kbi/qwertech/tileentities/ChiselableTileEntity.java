...
public class ChiselableTileEntity extends PrefixBlockTileEntity {
    ...
    public void onScheduledUpdate() {
        if (!(mBlocked = WD.visOcc(worldObj, xCoord, yCoord, zCoord, F, T))) {
            NW_API
                .sendToAllPlayersInRange(new PacketSyncDataIDs(getCoords(), mMetaData, mDesign), worldObj, getCoords());
    ...
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        if (mTexture == null) mTexture = (aBlock instanceof ChiselableBlock
            ? ((ChiselableBlock) aBlock).getTexture(mMetaData, mDesign, worldObj != null)
            : null);
        return mTexture;
    }
