...
public class UpgradeDesk extends TileEntityBase09FacingSingle
    implements IMultiTileEntity.IMTE_GetLightOpacity, IMultiTileEntity.IMTE_GetSubItems,
    IMultiTileEntity.IMTE_GetSelectedBoundingBoxFromPool, IMultiTileEntity.IMTE_AddCollisionBoxesToList,
    IMultiTileEntity.IMTE_GetBlocksMovement, IMultiTileEntity.IMTE_SetBlockBoundsBasedOnState {
                    ...
                    if (upgrade != null && upgrade.isCompatibleWith(currentOne)) {
                        ...
                        UT.Entities.chat(
                            ...
                            "You cannot place " + LH.Chat.BLINKING_RED
                                ...
                                + LH.Chat.WHITE
                                + " on "
                                + currentOne.getDisplayName());
                    }
                } else {
                    UT.Entities
                        .chat(aPlayer, "You cannot upgrade " + LH.Chat.BLINKING_RED + currentOne.getDisplayName());
                    // not armor
                }
            } else {
    ...
    public boolean onPlaced(ItemStack aStack, EntityPlayer aPlayer, MultiTileEntityContainer aMTEContainer,
        ...
        return super.onPlaced(
            ...
            aY,
            aZ,
            aSide == CS.SIDE_Y_NEG || aSide == CS.SIDE_Y_POS ? CS.SIDE_Z_POS : aSide,
            aHitX,
            aHitY,
            aHitZ);
    }
