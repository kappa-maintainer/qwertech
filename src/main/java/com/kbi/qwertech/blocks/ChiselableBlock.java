...
public class ChiselableBlock extends PrefixBlock implements IBlockSyncData.IBlockSyncDataAndIDs {
    ...
    public ChiselableBlock(ModData aMod, String aNameInternal, OreDictPrefix aPrefix, Drops aDrops, ITexture aTexture,
    ...
    public ChiselableBlock(String aModIDOwner, String aModIDTextures, String aNameInternal, OreDictPrefix aPrefix,
    ...
    public ChiselableBlock(ModData aMod, String aNameInternal, OreDictPrefix aPrefix,
    ...
    public void run() {
        if (mRegisterToOreDict) {
            for (short i = 0; i < mMaterialList.length; i++)
                if (mMaterialList[i] != null && mPrefix.isGeneratingItem(mMaterialList[i])) {
    ...
    public ITexture getTexture(short aMetaData, short aDesign, boolean aRendersInWorld) {
        ...
        if (mPrefix.mIconIndexBlock >= 0) {
            ...
            if (mTexture == null) {
                 ...
                 * return BlockTextureDefault.get(aMaterial, mPrefix, mCanGlow &&
                 * aMaterial.contains(TD.Properties.GLOWING));
                 * return BlockTextureDefault.get(MT.NULL, mPrefix, F);
                 */
            }
            // return BlockTextureMulti.get(BlockTextureDefault.get(aMaterial, mPrefix, mCanGlow &&
            // aMaterial.contains(TD.Properties.GLOWING)), BlockTextureDefault.get(icon[aDesign], CS.CA_WHITE, true,
            // false, false, true));
    ...
    public ITexture getTexture(int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered, IBlockAccess aWorld,
        ...
        return aShouldSideBeRendered[aSide]
            ? getTexture(getMetaDataValue(aWorld, aX, aY, aZ), getDesign(aWorld, aX, aY, aZ), T)
            : null;
    }
    ...
    public void setDesign(IBlockAccess aWorld, int aX, int aY, int aZ, short aMetaData, short aDesign) {
        ...
        if (aTileEntity == null && aWorld instanceof World) {
            aTileEntity = WD.te(
                ...
                aY,
                aZ,
                createTileEntity((World) aWorld, aX, aY, aZ, (byte) 6, aMetaData, nbt),
                F);
        } else {
    ...
    @Override
    public void receiveData(IBlockAccess aWorld, int aX, int aY, int aZ, INetworkHandler aNetworkHandler, short aID1,
        short aID2) {
